...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-navy-950/80 backdrop-blur-sm",
...
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-0 bg-navy-900 shadow-lift transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-400",
  {
    variants: {
      side: {
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn("mt-auto border-t border-white/[0.07] bg-navy-800/60 px-5 py-4", className)}
    {...props}
  />
);
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("heading-serif text-lg font-semibold", className)}
    {...props}
  />
));
